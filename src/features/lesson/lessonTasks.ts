...
function makeOddOneOutStep(focus: FocusItem[]): LessonStep | null {
  const chunks = focus
...
function makeSpotErrorStep(item: FocusItem): LessonStep | null {
  const clean = cleanHanzi(item.hanzi);
  ...
  if (stageId === "intro") {
    ...
    if (enablePedagogyVariants && practiceVariant !== "A") {
      for (const [index, item] of focus.entries()) {
        push(makeAudioDiscriminationStep(item, focus, (index + practiceVariant.charCodeAt(0)) % 2 === 0));
    ...
    if (enablePedagogyVariants) {
      ...
      if (practiceVariant === "C") {
        ...
        push(makeOddOneOutStep([...focus, ...reviewFocus]));
      }
    }
    for (const item of focus) {
    ...
    if (enablePedagogyVariants && practiceVariant === "B") {
      ...
      push(makeSpotErrorStep(primary));
    }
...
function generatedCandidatesFor(
  ...
  for (const stageId of LESSON_STAGE_ORDER) {
    ...
    const generated = supplementalStepsForStage(stageId, focus, target, { phaseOrder, reviewFocus, hanziBuilderProgress, seenGlyphs, ownFocusGlyphs, allowComposedFiller, unitIndex, allowImageSteps, sceneSelection, practiceVariant, enablePedagogyVariants: !lesson.isReview });
...
function selectPostConversationBlueprints(
  ...
  const tryPick = (bp: PostConversationBlueprint) => {
    ...
    if (usedKinds.has(kind)) return false;
    picked.push(bp);
