...
function StepWrite({ step, onDone, onSkip, onMistake }: StepProps) {
  ...
  return (
    <div>
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        used ? "border-line/50 bg-surface-2 text-ink-soft" : "",
                      ].join(" ")}
                    >
      ...
      {!composing && (
        // V4.9.5A.1 — produção escrita é produção: mesmo campo, mesmo mic, mesma
        // proposta de transcrição do resto do player.
        <FreeAnswerField
          value={draft}
          ...
          <div className="mt-2.5 flex flex-wrap justify-center gap-2" data-production-help-build>
            {bankTokens.map((token, index) => (
              <button
                ...
                className={[engineTileClass({ cjk: true, muted: usedIds.has(token.id) })]
                  .filter(Boolean)
                  .join(" ")}
              >
                {token.value}
...
function engineTileClass({
  ...
  wrong?: boolean;
  cjk?: boolean;
  /** Peça já usada: de-ênfase por token, nunca por véu de opacidade. */
  muted?: boolean;
}) {
  return [
    ...
    muted && !active && !matched && !wrong && "border-line/50 bg-surface-2 text-ink-soft",
          ...
          <div className="mt-2.5 flex flex-wrap justify-center gap-2" data-production-help-build>
            {bankTokens.map((token, index) => (
              <button
                ...
                className={[
                  engineTileClass({ cjk: isCjkText(token.value), muted: usedIds.has(token.id) }),
                ]
                  .filter(Boolean)
                  .join(" ")}
              >
