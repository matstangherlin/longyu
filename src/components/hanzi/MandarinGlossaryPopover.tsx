...
export function MandarinGlossaryPopover({
  ...
  const panel = mobile ? (
    <div
      ...
      className="animate-pop fixed inset-x-3 bottom-[calc(var(--app-bottom-nav-height)+0.75rem)] z-[110] overflow-y-auto overscroll-contain rounded-2xl border border-line bg-surface p-4 pr-11 text-left font-sans text-sm leading-normal text-ink shadow-lift lg:bottom-[calc(var(--app-safe-bottom)+1rem)]"
      style={{
        maxHeight:
          "min(72svh, calc(100svh - var(--app-safe-top) - var(--app-safe-bottom) - 7rem))",
      }}
      onPointerDown={stop}
      onClick={stop}
    >
