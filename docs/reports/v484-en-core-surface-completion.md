...
| Kind | What | Why |
| --- | --- | --- |
| PEDAGOGICAL_TARGET | Hanzi, pinyin, scored PT identity | Canonical Chinese / scoring |
| PROPER_NOUN | Longyu, Qi, XP, Pro, Mei, Dragon | Product names |
...
**REAL_UI_LEAKS_CORE_EN = 0** (static leftover gate + EN crawler).
...
| `npm run validate:i18n` | PASS (1744 catalog keys) |
...
| Playwright `e2e/en-core-surfaces.spec.ts` | recorded after the pack below |

Overlay key count after this wave: **2461** PT→EN strings in
`instructionGloss.en.json`. Product i18n version: **v4.8.4**.
