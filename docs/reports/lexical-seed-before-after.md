# Seed greetings — before × after (PED-021)

_Mesma definição de oportunidade pedagógica · baseline conteúdo `803e4b3` (#165) × branch atual_

Contagem: estímulo/resposta/montagem/falas primárias, dedupe por passo;
sem distractores de MCQ; sem stamps de vocabulário do planner;
你好吗 não conta como 你好.

## Totais L1–L20

| Seed | Antes | Depois | Δ |
|------|------:|-------:|--:|
| 你好 | 45 | 41 | -4 |
| 谢谢 | 13 | 12 | -1 |
| 再见 | 20 | 19 | -1 |
| 不客气 | 8 | 8 | 0 |
| 早上好 | 0 | 0 | 0 |
| 晚上好 | 0 | 0 | 0 |

## Destaque (critério de aceite)

- **你好** antes: **45** oportunidades
- **你好** depois: **41**
- **谢谢** antes: **13** · depois: **12**
- **再见** antes: **20** · depois: **19**
- **不客气** antes: **8** · depois: **8**

## Por lição (你好)

| Lição | Antes | Depois |
|-------|------:|-------:|
| `p1-o-que-e-mandarim` | 3 | 3 |
| `p1-o-que-e-pinyin` | 3 | 3 |
| `p1-o-que-e-tom` | 3 | 0 |
| `p1-o-que-e-hanzi` | 0 | 0 |
| `p1-primeiros-hanzi` | 0 | 0 |
| `p1-engine-2-lab` | 2 | 2 |
| `l1` | 4 | 4 |
| `l2` | 3 | 3 |
| `l3` | 2 | 2 |
| `l1-rev` | 4 | 4 |
| `l4` | 2 | 2 |
| `p1-ate-logo` | 3 | 2 |
| `p1-primeira-conversa` | 3 | 3 |
| `p1-qingwen-cortesia` | 2 | 2 |
| `l2-rev` | 5 | 5 |
| `p2-ma-primeiro-tom` | 1 | 2 |
| `p2-ma-segundo-tom` | 1 | 1 |
| `p2-ma-terceiro-tom` | 1 | 1 |
| `p2-ma-quarto-tom` | 1 | 1 |
| `p2-comparar-tom-1-4` | 2 | 1 |
