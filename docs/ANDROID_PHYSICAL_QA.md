...
5. `npm run android:install:debug`: instala com `adb install -r`, **só** em aparelho físico, e imprime o `versionName`/`versionCode` instalados de `com.longyu.app`.
...
| L | **Process death** | Com o app em background: `adb shell am kill com.longyu.app` (ou Opções do desenvolvedor → "Não manter atividades") → voltar. Nada corrompido, progresso salvo, conclusão e recompensa **não** duplicam |
...
| R | **Deep link** | `adb shell am start -W -a android.intent.action.VIEW -d "com.longyu.app://jornada"`, depois `…://hanzi/atlas` e `…://cultura`: o app abre na rota. `com.longyu.app://admin/feedback` é ignorado: o app abre, mas não navega para a rota proibida |
