#!/usr/bin/env node
/**
 * Dump localizable copy from teaching topics 51–80 (M1–M4).
 * Authoring helper for V4.8.5. Not a CI gate.
 */
...
const outJson = path.join(rootDir, "docs/localization/topics-51-80.dump.json");
const manifestJson = path.join(rootDir, "docs/localization/topics-51-80-manifest.json");
...
const outDir = await mkdtemp(path.join(os.tmpdir(), "longyu-t5180-dump-"));
try {
  ...
  const slice = teaching.slice(50, 80);
  if (slice.length !== 30) {
    console.error(`expected 30 topics 51–80, got ${slice.length} of ${teaching.length} teaching`);
    process.exit(1);
  }
    ...
    JSON.stringify(
      {
        ...
        rule: "ALL_LESSONS.filter(isTopicMasteryLesson).slice(50, 80)",
  ...
  console.log(slice.map((lesson, i) => `${i + 51}. ${lesson.id}\t${lesson.title}`).join("\n"));
} finally {
