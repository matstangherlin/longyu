...
exports.getLesson = exports.ALL_LESSONS = exports.JOURNEY = exports.FOUNDATION_LESSON_IDS = exports.TIERS = void 0;
exports.currentLessonId = currentLessonId;
exports.lessonState = lessonState;
exports.unitProgress = unitProgress;
...
const intro = (title, body) => ({ kind: "intro", title, body });
const listen = (text, pinyin, pt) => ({ kind: "listen", text, pinyin, pt });
const tone = (hanzi, pinyin, t, assist = "guided") => ({ kind: "tone", hanzi, pinyin, tone: t, assist });
const comp = (hanzi, pinyin, answer, options) => ({
    kind: "comprehend",
    hanzi,
    pinyin,
    answer,
...
const produce = (target, bank, pt) => ({ kind: "produce", target, bank, pt });
const write = (title, body, answer, placeholder = "Escreva sua resposta aqui", chunkId, guide = {}) => ({
    kind: "write",
    title,
    body,
    answer,
...
const recognize = (charId) => ({ kind: "recognize", charId });
const decompose = (charId) => ({ kind: "decompose", charId });
const hanziEvolution = (charIds, title, body) => ({
    kind: "hanzi_evolution",
    charIds,
    title,
    body,
});
const flash = (chunkId) => ({ kind: "flashcard", chunkId });
const read = (lines) => ({ kind: "microread", lines });
const dialogue = (title, dialoguePrompt, correctAnswer, options, explanation, speaker = "Situação") => ({
...
const sentenceBuild = (title, prompt, targetParts, bank, explanation) => ({
    kind: "sentence_build",
    title,
    prompt,
    targetParts,
...
const translationBuild = (title, sourceText, sourcePinyin, targetParts, bank, explanation, prompt = "Como fica em português?") => ({
...
const listenSelect = (title, audioText, options, correctAnswer, explanation, prompt = "Toque no que ouviu.") => ({
    kind: "listen_select",
    title,
    prompt,
    audioText,
...
const fillBlank = (title, prompt, sentenceBefore, blankAnswer, sentenceAfter, bank, explanation) => ({
    kind: "fill_blank",
    title,
    prompt,
    sentenceBefore,
...
const match = (title, body, pairs, explanation) => ({
    kind: "match_pairs",
    title,
    body,
    pairs,
...
const hanziBuild = (builderId, title, prompt, character, meaning, targetParts, bank) => ({
    kind: "hanzi_build",
    title,
    builderId,
    prompt,
...
const charIdByGlyph = new Map(characters_1.CHARACTERS.map((char) => [char.hanzi, char.id]));
function normalizeHanziRef(text) {
    return text.replace(/[，。！？、,.!?\s]/g, "");
}
function chunkRefByText(text) {
    if (!text)
        return undefined;
    const normalized = normalizeHanziRef(text);
    return chunks_1.CHUNKS.find((chunk) => normalizeHanziRef(chunk.hanzi) === normalized)?.id;
}
function refsFromText(text, refs) {
...
function refsFromSteps(steps) {
    ...
    for (const step of steps) {
        ...
        for (const line of step.lines ?? [])
            refsFromText(line.hanzi, refs);
    }
    return [...refs];
}
function estimateLessonMinutes(steps) {
    return Math.max(2, Math.min(6, Math.ceil(steps.length / 2)));
}
function withLessonDefaults(lesson) {
...
const microLesson = (lesson) => withLessonDefaults(lesson);
const review = (id, skill, steps, premium) => withLessonDefaults({
    id,
...
const PHASE1_BOOTSTRAP_LESSONS = [
    microLesson({
    ...
    microLesson({
        ...
        steps: [
            ...
            match("Pinyin e frase", "Combine som escrito e caracteres.", [
    ...
    microLesson({
        ...
        steps: [
            ...
            tone("妈", "mā", 1),
            tone("马", "mǎ", 3),
            match("Tom muda sentido", "Combine som e ideia.", [
            ...
            dialogue("Ideia principal", "Em mandarim, tom é...", "a curva da voz que pode mudar sentido", ["a curva da voz que pode mudar sentido", "a tradução em português", "o desenho do caractere", "o nome da pessoa"], "Tom é parte da pronúncia: mudar a curva pode mudar a palavra inteira.", "Escolha"),
        ],
    }),
    microLesson({
        id: "p1-o-que-e-hanzi",
        ...
        steps: [
            ...
            hanziEvolution(["mu", "ri", "yue", "ren"], "O que é hànzì?", "Cada hànzì carrega uma ideia. Veja quatro exemplos, um de cada vez — depois você monta o primeiro."),
            hanziBuild("hb-mu-fragments", "Monte seu primeiro hànzì", "Agora monte seu primeiro hànzì: 木 (árvore), encaixando as peças.", "木", "árvore / madeira"),
            match("Combine forma e ideia", "Combine cada hànzì com a ideia que ele carrega.", [
            ...
            match("Pinyin é som", "Agora combine hànzì, pinyin e significado.", [
            ...
            fillBlank("Complete a lógica", "Complete a composição: 木 + 木 + 木 = ___.", "木 + 木 + 木 = ", "森", "", ["森", "林", "明", "好"], "Três árvores formam 森, floresta densa. Você montará 林 e 明 depois, com mais prática."),
            dialogue("Reconheça a evolução", "Qual caractere tem relação com água?", "水", ["水", "口", "日", "人"], "水 significa água. Como radical lateral, costuma aparecer como 氵.", "Escolha"),
            dialogue("Revisão", "Hànzì é o quê?", "um sistema visual de caracteres chineses", ["um sistema visual de caracteres chineses", "um alfabeto de letras", "a tradução em português", "só um desenho antigo"], "Hànzì é escrita chinesa: visual, padronizada e ligada a som, sentido e uso.", "Escolha"),
            dialogue("Cuidado importante", "Todo hànzì moderno é apenas um desenho?", "não; muitos combinam uma peça de sentido e outra de som", ["não; muitos combinam uma peça de sentido e outra de som", "sim; todos são desenhos literais", "sim; todos são números", "não; hànzì é só pinyin"], "Nem todo caractere moderno é desenho. 妈, por exemplo, usa 女 como pista de sentido e 马 como pista sonora. Você montará composições como 林 e 明 em lições posteriores.", "Escolha"),
        ],
    }),
    microLesson({
        ...
        steps: [
            ...
            dialogue("Fechamento", "Como você montou os hànzì nesta lição?", "encaixando fragmentos simples", ["encaixando fragmentos simples", "juntando duas palavras inteiras", "soletrando letras", "copiando pinyin"], "Primeiro você aprende a montar blocos simples. Composições como 林 e 明 vêm depois.", "Escolha"),
        ],
    }),
];
const PHASE1_ENGINE_LESSONS = [
    microLesson({
        id: "p1-engine-2-lab",
...
const PHASE1_COURTESY_MICROTASKS = [
    microLesson({
        ...
        steps: [
            ...
            listenSelect("Escolha o que ouviu", "不客气", ["谢谢", "不客气", "你好", "再见"], "不客气", "不客气 é usado como de nada."),
            match("Cortesia em pares", "Combine cada frase com a função.", [
                { left: "谢谢", right: "Obrigado(a)", leftType: "hanzi", rightType: "pt" },
                { left: "不客气", right: "De nada", leftType: "hanzi", rightType: "pt" },
            ], "谢谢 agradece; 不客气 responde ao agradecimento."),
            sentenceBuild("Monte 不客气", "Monte: de nada.", ["不", "客气"], ["不", "客气", "谢谢", "你好"], "不客气 é uma resposta natural para 谢谢."),
            dialogue("Resposta curta", "Pessoa diz: 谢谢. O que você responde?", "不客气", ["不客气", "谢谢", "你好", "再见"], "不客气 é a resposta natural para de nada.", "Pessoa"),
            translationBuild("Escreva em português", "不客气", "bú kèqi", ["De", "nada"], ["nada", "Obrigado(a)", "De", "Olá"], "不客气 significa de nada."),
            listenSelect("Revisão rápida", "不客气", ["不客气", "谢谢"], "不客气", "Você ouviu 不客气."),
        ],
    }),
];
const PHASE2_MA_TONE_MICROTASKS = [
    microLesson({
    ...
    microLesson({
        ...
        steps: [
            ...
            tone("麻", "má", 2, "quiz"),
            dialogue("Descreva o som", "Como o 2º tom se move?", "sobe", ["sobe", "cai rápido", "fica reto", "desce e sobe"], "O 2º tom sobe, como uma pergunta curta.", "Escolha"),
        ],
    }),
    microLesson({
    ...
    microLesson({
        ...
        steps: [
            ...
            tone("骂", "mà", 4, "quiz"),
            dialogue("Queda rápida", "Qual tom cai rápido e firme?", "4º tom", ["4º tom", "1º tom", "2º tom", "3º tom"], "O 4º tom é a queda rápida, como um comando curto.", "Escolha"),
        ],
    }),
    microLesson({
        ...
        steps: [
            ...
            tone("妈", "mā", 1, "quiz"),
            tone("骂", "mà", 4, "quiz"),
            match("Compare os contornos", "Combine cada tom com o movimento da voz.", [
...
const PHASE2_CONTEXT_TONE_MICROTASKS = [
    microLesson({
        id: "p2-tons-nihao",
        title: "Tons em 你好",
        skill: "som",
...
const PHASE3_SURVIVAL_MICROTASKS = [
    microLesson({
        id: "p3-wohenhao",
        title: "我很好 — Estou bem",
        skill: "fala",
        ...
        steps: [
            ...
            listenSelect("Toque no que ouviu", "我很好", ["我很好", "你好吗？", "谢谢", "再见"], "我很好", "我很好 é a resposta simples para dizer que está bem."),
            match("Peças de 我很好", "Combine cada parte com o sentido.", [
            ...
            sentenceBuild("Monte a resposta", "Como dizer 'estou bem'?", ["我", "很", "好"], ["好", "你", "我", "很", "谢谢"], "我 = eu; 很好 = estou bem."),
            fillBlank("Complete a resposta", "Complete: 我 ___ 好.", "我", "很", "好", ["很", "吗", "叫", "是"], "我很好 = estou bem."),
            dialogue("Conversa curta", "Pessoa pergunta: 你好吗？ Qual resposta combina?", "我很好", ["我很好", "不客气", "再见", "你叫什么？"], "我很好 é uma resposta segura."),
            translationBuild("Revisão rápida", "我很好", "wǒ hěn hǎo", ["Estou", "bem"], ["Estou", "bem", "Obrigado", "Olá"], "我很好 = estou bem."),
        ],
    }),
    microLesson({
        ...
        steps: [
            ...
            listenSelect("Toque no que ouviu", "我不会说中文", ["我不会说中文", "我会说一点中文", "我很好", "谢谢"], "我不会说中文", "我不会说中文 é uma frase de proteção."),
            match("Peças de proteção", "Combine cada bloco com o sentido.", [
                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
            ...
            sentenceBuild("Não sei falar chinês", "Monte a frase de sobrevivência.", ["我", "不会", "说", "中文"], ["我", "不会", "会", "说", "中文", "一点"], "我不会说中文 = não sei falar chinês."),
            fillBlank("Complete a frase", "Complete: 我 ___ 说中文.", "我", "不会", "说中文", ["不会", "会", "很", "叫"], "我不会说中文 = não sei falar chinês."),
            dialogue("Conversa acelerou", "A pessoa fala rápido demais. Qual frase protege você?", "我不会说中文", ["我不会说中文", "谢谢", "我很好", "再见"], "Use 我不会说中文 quando você não sabe falar chinês."),
            translationBuild("Escreva em português", "我不会说中文", "wǒ bú huì shuō Zhōngwén", ["Não", "sei", "falar", "chinês"], ["falar", "Não", "obrigado", "sei", "chinês"], "我不会说中文 = não sei falar chinês."),
        ],
    }),
    microLesson({
        ...
        steps: [
            ...
            sentenceBuild("Peça ajuda", "Como pedir para a pessoa falar de novo?", ["请", "再", "说", "一遍"], ["请", "再", "说", "一遍", "谢谢", "中文"], "请再说一遍 = por favor, fale de novo."),
        ],
    }),
];
const PHASE4_CHARACTER_MICROTASKS = [
    microLesson({
        id: "p4-num-123",
        title: "一 二 三",
        skill: "hanzi",
...
const PHASE5_DECOMPOSITION_MICROTASKS = [
    microLesson({
        id: "p5-mu-mu-lin",
        title: "木 + 木 = 林",
        skill: "hanzi",
        ...
        steps: [
            ...
            match("Explique 林", "Combine a composição com o sentido.", [
    ...
    microLesson({
        ...
        steps: [
            ...
            match("Explique 森", "Combine a composição com o sentido.", [
    ...
    microLesson({
        ...
        steps: [
            ...
            match("Cena mental", "Combine a composição com a cena.", [
    ...
    microLesson({
        ...
        steps: [
            ...
            decompose("cong"),
            recognize("cong"),
            match("Siga a imagem", "Combine a composição com a ideia.", [
    ...
    microLesson({
        ...
        steps: [
            ...
            recognize("ma2"),
            tone("妈", "mā", 1, "quiz"),
            dialogue("Peça sonora", "Em 妈, qual peça dá o som ma?", "马", ["马", "女", "木", "口"], "马 dá a pista sonora ma; 女 dá o campo de sentido.", "Escolha"),
        ],
    }),
    microLesson({
        ...
        units: [
            {
                ...
                lessons: [
                    ...
                    ...PHASE1_ENGINE_LESSONS,
                    {
                        ...
                        steps: [
                            listen("你好", "nǐ hǎo", "Olá"),
                            listenSelect("Toque no que ouviu", "你好", ["你好", "谢谢", "再见", "不客气"], "你好", "你好 / nǐ hǎo é a saudação mais segura."),
                            match("Combine frase e sentido", "Ligue o cumprimento ao significado.", [
                                { left: "你好", right: "Olá", leftType: "hanzi", rightType: "pt" },
                            ...
                            sentenceBuild("Monte 你好", "Monte: Olá.", ["你", "好"], ["好", "你", "谢", "再"], "你 + 好 forma 你好."),
                            dialogue("Quando usar 你好?", "Você encontra alguém. O que combina dizer?", "你好", ["你好", "谢谢", "再见", "不客气"], "你好 é a saudação segura para encontrar ou cumprimentar alguém."),
                            fillBlank("Complete o cumprimento", "Complete a frase 你 ___.", "你", "好", "", ["好", "谢", "再", "见"], "你好 = olá."),
                            dialogue("Escolha o pinyin", "Qual pinyin combina com 你好?", "nǐ hǎo", ["nǐ hǎo", "xièxie", "zàijiàn", "bù kèqi"], "你好 se lê nǐ hǎo — dois 3º tons.", "Pinyin"),
                            translationBuild("Revisão rápida", "你好", "nǐ hǎo", ["Olá"], ["Obrigado(a)", "Olá", "Até", "logo"], "你好 significa olá."),
                        ],
                    },
                    {
                        id: "l2",
                        ...
                        steps: [
                            listen("谢谢", "xièxie", "Obrigado(a)"),
                            listenSelect("Distinguir agradecimento", "谢谢", ["你好", "谢谢", "再见", "不客气"], "谢谢", "谢谢 é a forma curta e comum de agradecer."),
                            match("Combine cortesia", "Ligue a frase ao sentido.", [
                                { left: "谢谢", right: "Obrigado(a)", leftType: "hanzi", rightType: "pt" },
                                { left: "谢", right: "agradecer", leftType: "hanzi", rightType: "pt" },
                            ], "谢谢 repete 谢 para agradecer."),
                            sentenceBuild("Monte 谢谢", "Monte: Obrigado(a).", ["谢", "谢"], ["谢", "你", "好"], "谢谢 repete 谢 para agradecer."),
                            dialogue("Escolha uma resposta", "Alguém te ajuda. Qual frase curta você usa?", "谢谢", ["谢谢", "你好", "再见", "不客气"], "谢谢 é a forma curta e comum de agradecer."),
                            tone("谢", "xiè", 4, "quiz"),
                            dialogue("Escolha o pinyin", "Qual pinyin combina com 谢谢?", "xièxie", ["xièxie", "nǐ hǎo", "zàijiàn", "bù kèqi"], "谢谢 começa com xiè, 4º tom na primeira sílaba.", "Pinyin"),
                            translationBuild("Escreva em português", "谢谢", "xièxie", ["Obrigado(a)"], ["Olá", "Obrigado(a)", "Até", "logo"], "谢谢 significa obrigado(a)."),
                            listenSelect("Revisão rápida", "谢谢", ["谢谢", "你好"], "谢谢", "Você ouviu 谢谢."),
                        ],
                    },
                    {
                        id: "l2-mapa-rapido",
                    ...
                    review("l1-rev", "fala", [
                        ...
                        dialogue("Escolha o pinyin", "Qual pinyin combina com 你好?", "nǐ hǎo", ["nǐ hǎo", "xièxie", "zàijiàn", "bù kèqi"], "你好 = nǐ hǎo.", "Pinyin"),
                        produce(["你", "好"], ["谢", "好", "你", "再"], "Olá"),
                        tone("谢", "xiè", 4, "quiz"),
                        comp("谢谢", "xièxie", "Obrigado(a)", ["Obrigado(a)", "Olá", "De nada", "Tchau"]),
                        match("Fixe com pares", "Combine frases antigas com o sentido.", [
                            { left: "你好", right: "Olá", leftType: "hanzi", rightType: "pt" },
                            { left: "谢谢", right: "Obrigado(a)", leftType: "hanzi", rightType: "pt" },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            listenSelect("Toque no que ouviu", "再见", ["你好", "谢谢", "再见", "不客气"], "再见", "再见 fecha a conversa."),
                            match("Combine despedida", "Ligue as peças ao sentido.", [
                                { left: "再见", right: "Até logo", leftType: "hanzi", rightType: "pt" },
                            ...
                            sentenceBuild("Monte 再见", "Monte: Até logo.", ["再", "见"], ["见", "谢", "再", "好"], "再 + 见 forma 再见."),
                            dialogue("Fechar uma conversa", "Você vai embora. O que combina dizer?", "再见", ["再见", "你好", "谢谢", "不客气"], "再见 fecha a conversa: até logo."),
                            translationBuild("Escreva em português", "再见", "zàijiàn", ["Até", "logo"], ["logo", "Obrigado(a)", "Até", "Olá"], "再见 significa até logo."),
                            listenSelect("Revisão rápida", "再见", ["再见", "谢谢"], "再见", "Você ouviu 再见."),
                            fillBlank("Frase antiga", "Complete o cumprimento que você já conhece.", "", "你好", "", ["你好", "再见", "谢谢", "不客气"], "你好 volta como saudação base."),
                        ],
                    },
                    ...PHASE1_COURTESY_MICROTASKS,
                    {
                        ...
                        steps: [
                            ...
                            dialogue("Escolha uma resposta para 谢谢", "Pessoa diz: 谢谢. O que você responde?", "不客气", ["不客气", "谢谢", "你好", "再见"], "不客气 é a resposta natural para de nada.", "Pessoa"),
                        ],
                    },
                    review("l2-rev", "fala", [
                        flash("nihao"),
                        ...
                        match("Revisão mista", "Combine frases antigas com o sentido.", [
                            { left: "你好", right: "Olá", leftType: "hanzi", rightType: "pt" },
                            { left: "再见", right: "Até logo", leftType: "hanzi", rightType: "pt" },
                            { left: "谢谢", right: "Obrigado(a)", leftType: "hanzi", rightType: "pt" },
                            { left: "不客气", right: "De nada", leftType: "hanzi", rightType: "pt" },
        ...
        units: [
            {
                ...
                lessons: [
                    ...PHASE2_MA_TONE_MICROTASKS,
                    {
                        ...
                        skill: "som",
                        steps: [
                            intro("Quatro contornos", "1º alto e reto · 2º sobe · 3º desce e sobe · 4º cai firme. A série ma (妈麻马骂) é o clássico para treinar."),
                            ...
                            tone("马", "mǎ", 3),
                            tone("骂", "mà", 4),
                            match("Tom muda sentido", "Combine cada sílaba com o sentido.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            dialogue("Dica auditiva", "Qual tom parece uma queda rápida e firme?", "4º tom", ["4º tom", "1º tom", "2º tom", "3º tom"], "O 4º tom cai rápido e firme, como um comando curto.", "Escolha"),
                        ],
                    },
                    review("l3-rev", "som", [
                        tone("妈", "mā", 1, "quiz"),
                        ...
                        match("Compare dois tons", "Combine cada tom com o contorno.", [
                            { left: "1º tom", right: "alto e reto", leftType: "pt", rightType: "pt" },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            match("Ouvido antes da memória", "Combine cada sílaba com o contorno.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            comp("你好", "nǐ hǎo", "Olá", ["Olá", "Obrigado(a)", "Até logo", "De nada"]),
                            tone("谢", "xiè", 4),
                            dialogue("Conecte som e frase", "Em 你好, qual sílaba tem 3º tom?", "好 / hǎo", ["好 / hǎo", "谢 / xiè", "是 / shì", "再 / zài"], "好 / hǎo tem 3º tom.", "Escolha"),
                        ],
                    },
                    {
                        id: "l8-compare",
                        ...
                        steps: [
                            ...
                            tone("麻", "má", 2, "quiz"),
                            tone("马", "mǎ", 3, "quiz"),
                            match("Dois pares", "Combine cada tom com o contorno.", [
                                { left: "1º tom", right: "alto e reto", leftType: "pt", rightType: "pt" },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            listenSelect("Toque no que ouviu", "我叫马修", ["我叫马修", "我是巴西人", "我很好", "谢谢"], "我叫马修", "我叫 + nome apresenta quem você é."),
                            match("Peças da apresentação", "Combine cada parte com o sentido.", [
                                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
                            ...
                            sentenceBuild("Meu nome é...", "Monte em mandarim: meu nome é Matheus.", ["我", "叫", "马修"], ["我", "叫", "是", "你好", "马修"], "我叫 + nome é a forma curta para se apresentar."),
                            dialogue("Responda a pergunta", "Alguém pergunta: 你叫什么？ Como você responde?", "我叫马修", ["我叫马修", "谢谢", "再见", "不客气"], "Use 我叫 + seu nome para responder."),
                            translationBuild("Escreva em português", "我叫马修", "wǒ jiào Mǎxiū", ["Meu", "nome", "é", "Matheus"], ["Matheus", "Meu", "sou", "nome", "é"], "我叫马修 = meu nome é Matheus."),
                            fillBlank("Complete a apresentação", "Complete: 我 ___ 马修.", "我", "叫", "马修", ["叫", "是", "好", "谢"], "我叫马修 = eu me chamo Matheus."),
                        ],
                    },
                    {
                        id: "l9-tudo-bem",
                        ...
                        steps: [
                            ...
                            fillBlank("Complete a pergunta", "Complete: 你 ___ 吗？", "你", "好", "吗？", ["好", "叫", "谢", "见"], "你好吗？ pergunta se a pessoa está bem."),
                            sentenceBuild("Monte a pergunta", "Monte: Tudo bem?", ["你", "好", "吗"], ["吗", "你", "好", "我", "很"], "你 + 好 + 吗 forma 你好吗？"),
                            dialogue("Escolha a resposta", "Pessoa pergunta: 你好吗？ O que você responde se está bem?", "我很好", ["我很好", "再见", "谢谢", "我叫马修"], "我很好 responde: estou bem."),
                            match("Pergunta e resposta", "Combine cada frase com o sentido.", [
                                { left: "你好吗？", right: "Tudo bem?", leftType: "hanzi", rightType: "pt" },
                                { left: "我很好", right: "Estou bem", leftType: "hanzi", rightType: "pt" },
                            ], "你好吗？ pergunta; 我很好 responde."),
                            listenSelect("Ouça a resposta", "我很好", ["你好吗？", "我很好", "谢谢", "再见"], "我很好", "Você ouviu 我很好."),
                            translationBuild("Revisão rápida", "我很好", "wǒ hěn hǎo", ["Estou", "bem"], ["Tudo", "Estou", "bem", "Obrigado"], "我很好 = estou bem."),
                        ],
                    },
                    {
                        id: "l9-qual-nome",
                        ...
                        steps: [
                            ...
                            sentenceBuild("Responda com seu nome", "Você ouviu 你叫什么？ Monte a resposta.", ["我", "叫", "马修"], ["我", "叫", "什么", "马修", "你好"], "我叫 + nome responde como você se chama."),
                        ],
                    },
                    {
                        id: "l10",
                        ...
                        steps: [
                            ...
                            listenSelect("Toque no que ouviu", "我是巴西人", ["我是巴西人", "我叫马修", "我很好", "我听不懂"], "我是巴西人", "我是巴西人 diz sou brasileiro."),
                            match("Mapa da frase", "Combine cada peça com o sentido.", [
                                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
                            ...
                            sentenceBuild("Sou brasileiro", "Como você diria: “Eu sou brasileiro”?", ["我", "是", "巴西人"], ["是", "我", "你好", "巴西人", "叫"], "我 = eu, 是 = ser/sou, 巴西人 = brasileiro."),
                            fillBlank("Complete a origem", "Complete: 我 ___ 巴西人.", "我", "是", "巴西人", ["是", "叫", "很", "吗"], "我是巴西人 = sou brasileiro."),
                            dialogue("Apresente sua origem", "Alguém pergunta de onde você é. Qual frase responde?", "我是巴西人", ["我是巴西人", "我很好", "谢谢", "再见"], "我是巴西人 responde sua origem."),
                            translationBuild("Escreva em português", "我是巴西人", "wǒ shì Bāxī rén", ["Sou", "brasileiro"], ["Sou", "Obrigado", "brasileiro", "bem"], "我是巴西人 = sou brasileiro."),
                        ],
                    },
                    ...PHASE3_SURVIVAL_MICROTASKS,
                    {
                        ...
                        steps: [
                            ...
                            listenSelect("Toque no que ouviu", "我听不懂", ["我听不懂", "我不会说中文", "我很好", "谢谢"], "我听不懂", "我听不懂 é a frase curta para não entendi."),
                            match("O que quebrou?", "Combine cada frase com a situação.", [
                            ...
                            sentenceBuild("Não entendo", "Em uma conversa rápida, monte: “Não entendo”.", ["我", "听不懂"], ["我", "听不懂", "会", "说", "中文", "谢谢"], "我听不懂 é a frase curta para quando você não entendeu o que ouviu."),
                            fillBlank("Complete a frase", "Complete: 我 ___ .", "我", "听不懂", "", ["听不懂", "很好", "谢谢", "再见"], "我听不懂 = não entendi."),
                            dialogue("Peça reparo", "Você não entendeu o que ouviu. Qual frase combina?", "我听不懂", ["我听不懂", "我很好", "不客气", "再见"], "我听不懂 comunica que você não entendeu."),
                            translationBuild("Escreva em português", "我听不懂", "wǒ tīng bù dǒng", ["Não", "entendi"], ["Não", "obrigado", "entendi", "bem"], "我听不懂 = não entendi."),
                        ],
                    },
                    {
                        id: "l11-falo-pouco",
                        ...
                        steps: [
                            ...
                            listenSelect("Toque no que ouviu", "我会说一点中文", ["我会说一点中文", "我不会说中文", "我听不懂", "谢谢"], "我会说一点中文", "会 fala de habilidade; 一点 suaviza para um pouco."),
                            match("Blocos da frase", "Combine cada bloco com o sentido.", [
                                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
                            ...
                            sentenceBuild("Falo um pouco de chinês", "Como você diria “sei falar um pouco de chinês”?", ["我", "会", "说", "一点", "中文"], ["我", "会", "说", "一点", "中文", "不会", "听不懂"], "我会说一点中文 = sei falar um pouco de chinês."),
                            fillBlank("Complete o meio", "Complete: 我会说 ___ 中文.", "我会说", "一点", "中文", ["一点", "不懂", "很好", "再见"], "一点 deixa a frase mais modesta: um pouco."),
                            dialogue("Seja honesto no nível", "Você sabe só um pouco. Qual frase combina?", "我会说一点中文", ["我会说一点中文", "我不会说中文", "再见", "不客气"], "Essa frase ajuda a ajustar a expectativa da conversa."),
                            translationBuild("Escreva em português", "我会说一点中文", "wǒ huì shuō yìdiǎn Zhōngwén", ["Sei", "falar", "um", "pouco", "de", "chinês"], ["falar", "Sei", "de", "chinês", "um", "pouco", "Obrigado"], "我会说一点中文 = sei falar um pouco de chinês."),
                        ],
                    },
                ],
            },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            match("Mapa da frase", "Combine cada peça com o papel na frase.", [
                                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
                                { left: "是", right: "sou / ser", leftType: "hanzi", rightType: "pt" },
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            flash("zaijian"),
                            match("Reconheça no texto", "Combine as frases do microtexto.", [
                                { left: "你好！", right: "Olá!", leftType: "hanzi", rightType: "pt" },
                                { left: "谢谢。", right: "Obrigado(a).", leftType: "hanzi", rightType: "pt" },
                                { left: "再见！", right: "Até logo!", leftType: "hanzi", rightType: "pt" },
                            ], "O texto usa frases que você já praticou em produção."),
                            translationBuild("Despedida do texto", "再见！", "Zàijiàn!", ["Até", "logo!"], ["Até", "Obrigado(a).", "logo!", "Olá!"], "再见 fecha o microtexto."),
                            comp("我是巴西人。", "Wǒ shì Bāxī rén.", "Sou brasileiro.", ["Sou brasileiro.", "Meu nome é Matheus.", "Obrigado(a).", "Até logo."]),
                            translationBuild("Entendeu o texto?", "我是巴西人。", "Wǒ shì Bāxī rén.", ["Eu", "sou", "brasileiro."], ["brasileiro.", "Eu", "Obrigado.", "sou"], "我是巴西人 comunica: eu sou brasileiro."),
                            listenSelect("Revisão de leitura", "再见", ["你好", "谢谢", "再见", "我是巴西人"], "再见", "再见 fecha o microtexto."),
                        ],
                    },
                    {
                        id: "l13-dialogo-ola",
                        ...
                        steps: [
                            ...
                            translationBuild("Seu primeiro diálogo", "你好！ 你好吗？ 我很好。 谢谢。", "Nǐ hǎo! Nǐ hǎo ma? Wǒ hěn hǎo. Xièxie.", ["Olá.", "Tudo bem?", "Estou bem.", "Obrigado(a)."], ["Estou bem.", "Olá.", "Até logo.", "Tudo bem?", "Obrigado(a)."], "A ordem é saudação, pergunta, resposta e agradecimento."),
                        ],
                    },
                    {
                        id: "l13-dialogo-nome",
                        ...
                        steps: [
                            ...
                            sentenceBuild("Resposta natural", "Como você responderia 你叫什么？", ["我", "叫", "马修"], ["我", "叫", "马修", "你", "什么"], "我叫 + nome responde “eu me chamo...”."),
                        ],
                    },
                    review("l5-rev", "fala", [
                        flash("nihaoma"),
                        ...
                        sentenceBuild("Produção guiada", "Monte uma apresentação curta com saudação e nome.", ["你好", "我", "叫", "马修"], ["你好", "我", "叫", "马修", "是", "巴西人"], "你好 abre a conversa; 我叫 + nome apresenta você."),
                    ]),
                ],
            },
        ],
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            match("Peça, não desenho", "Combine cada radical com a ideia que ele costuma sugerir.", [
                                { left: "人", right: "pessoa", leftType: "hanzi", rightType: "pt" },
                    ...
                    ...PHASE4_CHARACTER_MICROTASKS,
                    {
                        ...
                        steps: [
                            ...
                            recognize("wu"),
                            produce(["一", "二", "三"], ["三", "一", "五", "二"], "um, dois, três"),
                            match("Primeira lógica visual", "Combine o caractere com a quantidade de traços.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            recognize("huo"),
                            recognize("shui"),
                            match("Agrupe por imagem", "Combine cada peça com a imagem principal.", [
                                { left: "日", right: "sol / dia", leftType: "hanzi", rightType: "pt" },
                                { left: "月", right: "lua / mês", leftType: "hanzi", rightType: "pt" },
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            match("Reconheça sem traduzir tudo", "Combine as peças mais úteis da frase.", [
                                { left: "我", right: "eu", leftType: "hanzi", rightType: "pt" },
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            recognize("lin"),
                            recognize("sen"),
                            match("Repetição visual", "Combine cada forma com a ideia.", [
                                { left: "木", right: "árvore", leftType: "hanzi", rightType: "pt" },
                                { left: "林", right: "bosque", leftType: "hanzi", rightType: "pt" },
                                { left: "森", right: "floresta", leftType: "hanzi", rightType: "pt" },
                    ...
                    review("l6-rev", "hanzi", [
                        ...
                        recognize("sen"),
                        decompose("sen"),
                        match("Desmonte mentalmente", "Combine cada caractere com o sentido.", [
                            { left: "木", right: "árvore", leftType: "hanzi", rightType: "pt" },
                            { left: "林", right: "bosque", leftType: "hanzi", rightType: "pt" },
                            { left: "森", right: "floresta", leftType: "hanzi", rightType: "pt" },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            recognize("ma2"),
                            tone("妈", "mā", 1, "quiz"),
                            dialogue("Peça de som", "Em 妈, qual peça dá pista sonora?", "马", ["马", "女", "木", "日"], "马 dá a pista sonora ma. 女 dá o campo de sentido.", "Escolha"),
                        ],
                    },
                    {
                        id: "l17",
                        ...
                        steps: [
                            ...
                            match("Sol + lua", "Combine as peças de 明.", [
                                { left: "日", right: "sol / dia", leftType: "hanzi", rightType: "pt" },
                                { left: "月", right: "lua / mês", leftType: "hanzi", rightType: "pt" },
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            recognize("peng"),
                            sentenceBuild("Monte 朋友", "Monte: amigo.", ["朋", "友"], ["友", "朋", "明", "妈"], "朋友 junta dois caracteres para formar amigo."),
                            dialogue("Da peça à palavra", "Qual é a palavra de dois caracteres para amigo?", "朋友", ["朋友", "中国", "谢谢", "你好"], "朋友 / péngyou significa amigo.", "Escolha"),
                        ],
                    },
                ...
                lessons: [
                    ...PHASE5_DECOMPOSITION_MICROTASKS,
                    {
                        ...
                        steps: [
                            ...
                            recognize("sen"),
                            recognize("xiu"),
                            match("Leia a lógica", "Combine composição e significado.", [
                                { left: "木 + 木 + 木", right: "森", leftType: "hanzi", rightType: "hanzi" },
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            match("Peças com sentido", "Combine cada caractere com a composição.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            recognize("cong"),
                            recognize("zhong3"),
                            match("Cena mental", "Combine a composição com o sentido.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            match("Sentido ou som?", "Combine as peças de 吗 com seus papéis.", [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            decompose("ma2"),
                            decompose("ma_question"),
                            match("Peças favoritas", "Combine as combinações já vistas.", [
                                { left: "日 + 月", right: "明", leftType: "hanzi", rightType: "hanzi" },
                                { left: "木 + 木", right: "林", leftType: "hanzi", rightType: "hanzi" },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            recognize("san"),
                            listen("四", "sì", "quatro"),
                            listenSelect("Ouça o número", "四", ["一", "二", "四", "五"], "四", "四 é quatro."),
                            recognize("si"),
                            comp("五", "wǔ", "cinco", ["cinco", "quatro", "três", "um"]),
                            recognize("wu"),
                            produce(["一", "二", "三"], ["三", "一", "五", "二"], "um, dois, três"),
                            sentenceBuild("Monte 1 a 5", "Monte a sequência de um a cinco.", ["一", "二", "三", "四", "五"], ["三", "一", "五", "二", "四"], "一 二 三 四 五 é a primeira sequência numérica."),
                            fillBlank("Complete a sequência", "Complete: 一 二 三 ___ 五.", "一二三", "四", "五", ["四", "二", "五", "三"], "四 vem depois de 三 e antes de 五."),
                            dialogue("Número seguinte", "Na sequência 一 二 三 四, qual vem depois?", "五", ["五", "二", "三", "一"], "五 fecha a sequência de 1 a 5.", "Escolha"),
                            match("Lógica visual", "Combine o caractere com a quantidade.", [
                    ...
                    review("l8-rev", "hanzi", [
                        ...
                        dialogue("Revisão de números", "Qual caractere significa dez?", "十", ["十", "九", "三", "人"], "十 significa dez.", "Escolha"),
                    ]),
                ],
            },
            {
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            dialogue("Peça que muda o grupo", "Qual peça transforma 我 em 我们?", "们", ["们", "人", "是", "不"], "们 transforma eu/você em nós/vocês: 我 → 我们, 你 → 你们.", "Escolha"),
                        ],
                    },
                    {
                        id: "l22",
                        ...
                        steps: [
                            ...
                            dialogue("Composto útil", "Qual palavra significa amigo?", "朋友", ["朋友", "中国", "我们", "中文"], "朋友 / péngyou significa amigo.", "Escolha"),
                        ],
                    },
                    {
                        id: "l23",
                        ...
                        steps: [
                            ...
                            translationBuild("Resumo do texto", "你好！我叫马修。我是巴西人。我有三个朋友。", "Nǐ hǎo! Wǒ jiào Mǎxiū. Wǒ shì Bāxī rén. Wǒ yǒu sān ge péngyou.", ["Olá.", "Meu nome é Matheus.", "Sou brasileiro.", "Tenho três amigos."], ["Sou brasileiro.", "Obrigado.", "Olá.", "Tenho três amigos.", "Meu nome é Matheus."], "O texto cumprimenta, apresenta nome, origem e três amigos."),
                        ],
                    },
                    review("l9-rev", "fala", [
                        flash("women"),
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            sentenceBuild("Apresente seu pai", "Monte: Este é meu pai.", ["这", "是", "我", "爸爸"], ["我", "爸爸", "这", "妈妈", "是"], "这是我爸爸 apresenta quem é seu pai."),
                            fillBlank("Apresente sua mãe", "Complete: 这是我 ___.", "这是我", "妈妈", "", ["妈妈", "爸爸", "朋友", "中文"], "这是我妈妈 apresenta quem é sua mãe."),
                            dialogue("Foto da família", "Alguém aponta para seu pai na foto. O que você diz?", "这是我爸爸", ["这是我爸爸", "这是我妈妈", "这是什么？", "我喜欢中文"], "这是我爸爸 apresenta seu pai em uma foto.", "Pessoa"),
                            dialogue("Outra foto", "Alguém aponta para sua mãe. O que você diz?", "这是我妈妈", ["这是我妈妈", "这是我爸爸", "我要这个", "我想喝茶"], "这是我妈妈 apresenta sua mãe.", "Pessoa"),
                        ],
                    },
                    {
                        id: "l25",
                        ...
                        steps: [
                            ...
                            listenSelect("Ouça a pergunta", "这是什么？", ["这是什么？", "这是我爸爸", "你好", "你叫什么？"], "这是什么？", "这是什么？ pergunta o que é algo."),
                            comp("这是什么？", "zhè shì shénme?", "O que é isto?", ["O que é isto?", "Onde fica?", "Quanto custa?", "Com licença"]),
                            sentenceBuild("Monte a pergunta", "Monte: O que é isto?", ["这", "是", "什么"], ["什么", "这", "是", "我", "爸爸"], "这是什么？ pergunta o que é algo."),
                            dialogue("Objeto desconhecido", "Você vê um objeto e não sabe o que é. Qual pergunta combina?", "这是什么？", ["这是什么？", "这是我妈妈", "我喜欢中文", "我们走吧"], "Use 这是什么？ para perguntar o que é algo.", "Situação"),
                        ],
                    },
                ],
            },
                ...
                lessons: [
                    {
                        ...
                        steps: [
                            ...
                            sentenceBuild("Monte seu gosto", "Monte: Eu gosto de chinês.", ["我", "喜欢", "中文"], ["中文", "我", "喜欢", "想", "茶"], "我喜欢中文 diz uma preferência simples."),
                            dialogue("Preferência", "Você quer dizer que gosta de chinês. Qual frase combina?", "我喜欢中文", ["我喜欢中文", "我想喝茶", "我要这个", "谢谢"], "我喜欢中文 expressa gosto ou interesse.", "Situação"),
                        ],
                    },
                    {
                        id: "l27",
                        ...
                        steps: [
                            ...
                            listenSelect("Ouça o pedido", "我想喝茶", ["我想喝茶", "我要这个", "多少钱？", "我喜欢中文"], "我想喝茶", "我想喝茶 pede chá como desejo."),
                            listenSelect("Ouça a pergunta de preço", "多少钱？", ["多少钱？", "我要这个", "我想喝茶", "我们走吧"], "多少钱？", "多少钱？ pergunta o preço."),
                            ...
                            sentenceBuild("Pergunte o preço", "Monte: Quanto custa?", ["多少", "钱"], ["钱", "这个", "多少", "茶"], "多少钱？ é a pergunta curta de preço."),
                            sentenceBuild("Escolha o item", "Monte: Eu quero este.", ["我", "要", "这个"], ["这个", "我", "要", "多少", "钱"], "我要这个 resolve uma compra simples."),
                            fillBlank("Complete o pedido", "Complete: 我 ___ 这个.", "我", "要", "这个", ["要", "想", "喝", "喜欢"], "我要这个 = eu quero este."),
                            dialogue("Na barraca", "Você aponta para um produto e quer saber o preço. O que pergunta?", "多少钱？", ["多少钱？", "我要这个", "我喜欢中文", "我们走吧"], "多少钱？ é a pergunta mais direta para preço.", "Vendedor"),
                            dialogue("Depois do preço", "O preço está bom e você quer comprar. O que diz?", "我要这个", ["我要这个", "多少钱？", "我想喝茶", "这是我爸爸"], "我要这个 confirma que você quer aquele item.", "Vendedor"),
                            produce(["我", "想", "喝", "茶"], ["茶", "我", "喝", "想", "吃"], "Quero beber chá"),
                            write("Escreva em português", "Escreva em português: 我想喝茶", "Quero beber chá", "Monte a tradução com as peças", "woxianghe", {
                ...
                lessons: [
                    ...
                    },
                    {
                        ...
                        steps: [
                            ...
                            sentenceBuild("Reconstrua o gosto", "Monte a linha: Eu gosto de chinês.", ["我", "喜欢", "中文"], ["中文", "喜欢", "我", "茶", "朋友"], "我喜欢中文 é uma linha curta de gosto."),
                            sentenceBuild("Reconstrua o pedido", "Monte a linha: Quero beber chá.", ["我", "想", "喝", "茶"], ["茶", "想", "我", "喝", "走"], "我想喝茶 transforma leitura em fala útil."),
                            dialogue("Fechar a cena", "Depois do chá e da conversa, qual frase encerra a cena?", "我们走吧", ["我们走吧", "我喜欢中文", "我想喝茶", "我有三个朋友"], "我们走吧 fecha a cena: vamos embora.", "Narrador"),
                        ],
                    },
                    review("l11-rev", "leitura", [
                        ...
                        sentenceBuild("Tenho três amigos", "Monte a frase do texto.", ["我", "有", "三", "个", "朋友"], ["朋友", "三", "个", "我", "有", "喜欢"], "我有三个朋友 reaparece como produção guiada."),
                    ], true),
                ],
            },
        ],
...
exports.ALL_LESSONS = exports.JOURNEY.flatMap((p) => p.units.flatMap((u) => u.lessons.map((l) => {
...
const getLesson = (id) => exports.ALL_LESSONS.find((l) => l.id === id);
exports.getLesson = getLesson;
function currentLessonId(completed, _isPremium = false) {
    return exports.ALL_LESSONS.find((l) => !completed.includes(l.id))?.id;
}
function lessonState(id, completed, isPremium = false) {
    if (completed.includes(id))
        return "done";
    const lesson = (0, exports.getLesson)(id);
...
function unitProgress(unit, completed) {
    const total = unit.lessons.length;
    const done = unit.lessons.filter((l) => completed.includes(l.id)).length;
    return { done, total };
}
function getPhaseById(id) {
    return exports.JOURNEY.find((p) => p.id === id);
}
