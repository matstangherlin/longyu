#!/usr/bin/env node
/**
 * Dump localizable copy from teaching topics 81–113 (M1–M4).
 * Authoring helper for V4.8.7. Not a CI gate.
 */
...
const outJson = path.join(rootDir, "docs/localization/topics-81-113.dump.json");
const manifestJson = path.join(rootDir, "docs/localization/topics-81-113-manifest.json");
...
const outDir = await mkdtemp(path.join(os.tmpdir(), "longyu-t81113-dump-"));
try {
  ...
  if (program.emit().emitSkipped) {
    console.error("dump-topics-51-80: TypeScript emit failed");
    process.exit(1);
  }
  ...
  const slice = teaching.slice(80, 113);
  if (slice.length !== 33) {
    console.error(`expected 33 topics 81–113, got ${slice.length} of ${teaching.length} teaching`);
    process.exit(1);
  }
  ...
  const unique = [...strings.entries()]
    .map(([pt, fields]) => ({
      pt,
      fields: [...fields].sort(),
      alreadyInGloss: Boolean(existingGloss[pt]),
    }))
  ...
  const payload = {
    ...
    newStringCount: unique.filter((row) => !row.alreadyInGloss).length,
    reusedGlossCount: unique.filter((row) => row.alreadyInGloss).length,
    topics,
    lexical,
    strings: unique,
  };
    ...
    JSON.stringify(
      {
        ...
        rule: "ALL_LESSONS.filter(isTopicMasteryLesson).slice(80, 113)",
  ...
  console.log(slice.map((lesson, i) => `${i + 81}. ${lesson.id}\t${lesson.title}`).join("\n"));
} finally {
