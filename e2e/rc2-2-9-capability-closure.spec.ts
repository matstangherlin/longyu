 ...
 *
 * O seed só leva o aluno ao ponto pedagógico (lições anteriores concluídas,
 * rodada de maestria certa e, quando o passo é o último da rodada, o cursor de
 * retomada que o próprio player usa). A atividade testada é sempre a que
 * `lessonRoundStepsFor` entrega — nada aqui injeta passo.
 *
...
/**
 * Abre a lição na rodada `pass`. Com `atEnd`, usa o cursor de retomada do
 * player para cair no último passo da rodada (onde entram os passos de
 * fechamento); sem ele, o teste caminha pela rodada até o passo.
 */
async function openLesson(page: Page, lessonId: string, pass: number, options: { atEnd?: boolean } = {}) {
  await seedUnlockedLessonSession(page, lessonId, {
    lessonMasteryById: masteryUpTo(lessonId, pass),
    ...(options.atEnd ? { lessonSessionStepById: { [lessonId]: { pass, stepIndex: 999 } } } : {}),
  });
...
async function answerProduction(page: Page, situation: string, answer: string) {
  ...
  await expect(page.getByRole("status").filter({ hasText: /Boa! \+Qi|Nice! \+Qi/ }).first()).toBeVisible({ timeout: 15_000 });
}
...
async function assemble(page: Page, title: string, pieces: string[]) {
  ...
  await expect(board.getByText("+Qi").first()).toBeVisible({ timeout: 15_000 });
}
...
async function hearAndChoose(page: Page, title: string, heard: string, meaning: string) {
  ...
  await expect(page.getByRole("status").filter({ hasText: /Boa! \+Qi|Nice! \+Qi|Certo|Correct/ }).first()).toBeVisible({ timeout: 15_000 });
}

/** Na cena aberta, avança as falas até a intervenção e escolhe a resposta. */
async function replyInScene(scene: Locator, prompt: string, reply: string) {
  const option = scene.getByRole("button", { name: new RegExp(`^(Opção|Option) \\d+: ${reply.replace(/[.*+?^${}()|[\]\\！]/g, "\\$&")}$`) });
  const deadline = Date.now() + 30_000;
  while (!(await option.first().isVisible().catch(() => false)) && Date.now() < deadline) {
  ...
  await expect(scene.getByText(prompt).first()).toBeVisible();
...
test.describe("RC2.2.9 — capacidades conversacionais fechadas em runtime", () => {
  ...
  test("talk_family — produção + transferência na visita à casa (visit-home)", async ({ page }) => {
    await openLesson(page, "p7-imersao-casa-amigo", 3, { atEnd: true });
  ...
  test("order_food — escuta: entende a restrição 不要辣 pelo áudio", async ({ page }) => {
    await openLesson(page, "l26b", 2, { atEnd: true });
  ...
  test("order_food — produz 不要辣 numa situação, sem alternativas", async ({ page }) => {
    await openLesson(page, "l26b", 4, { atEnd: true });
  ...
  test("negotiate_basic — escuta: entende o pedido de desconto", async ({ page }) => {
    await openLesson(page, "l27", 2, { atEnd: true });
  ...
  test("negotiate_basic — pechincha numa situação nova", async ({ page }) => {
    await openLesson(page, "l27", 4, { atEnd: true });
  ...
  test("pay — 可以刷卡吗 é ensinado antes de ser cobrado", async ({ page }) => {
    await openLesson(page, "l27", 1, { atEnd: true });
    await expect(page.locator('[data-step-kind="flashcard"]').first()).toContainText("可以刷卡吗");
  });
  ...
  test("express_preference — recusa um prato numa situação nova", async ({ page }) => {
    await openLesson(page, "l28", 4, { atEnd: true });
  ...
  test("make_simple_plan — quando + destino numa conversa cotidiana", async ({ page }) => {
    await openLesson(page, "p7-conversa-cotidiana", 1, { atEnd: true });
