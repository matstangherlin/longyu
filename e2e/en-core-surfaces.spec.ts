...
test.describe("V4.8.4 English core surfaces", () => {
  test("i18n version is visible on the document for stale-build diagnosis", async ({ page }) => {
    ...
    await expect(page.locator("html")).toHaveAttribute("data-i18n-version", "v4.8.8");
    const sw = await page.request.get("/sw.js");
    if (sw.ok()) {
      const body = await sw.text();
      expect(body, "PWA cacheId must include the i18n version").toContain("longyu-i18n-v4.8.8");
    }
  });
